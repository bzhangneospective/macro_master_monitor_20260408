...
# ==========================================
st.set_page_config(
    page_title="Macro Terminal V2.1", 
    layout="wide", 
...
if db:
    ...
    if page == "📈 Equity Markets":
        ...
        with tab2:
            ...
            with c1:
                us_data = {
                    "Sector": ["Energy", "Shipping", "Consumer Staples", "Materials", "Industrials", "Health Care", "Software", "Semiconductors"], 
                    "YTD (%)": [25.7, 23.3, 21.8, 10.3, 8.0, -1.2, 6.5, -12.1]
                }
                us_sec = pd.DataFrame(us_data)
            ...
            with c2:
                hk_data = {
                    "Sector": ["HSCEI ETF", "China Internet", "CSI 300 HK", "HS China Ent", "HS Index ETF", "HS Tech ETF"], 
                    "YTD (%)": [-12.5, -10.0, -7.5, -5.2, -5.0, -2.5]
                }
                hk_sec = pd.DataFrame(hk_data)
            ...
            with c3:
                cn_data = {
                    "Sector": ["Tech", "CSI 500", "Real Estate", "Gaming", "Bank", "ChiNext", "Pharma", "Biotech", "5G", "Dividend", "Military", "Coal"], 
                    "YTD (%)": [9.3, 8.7, 5.8, 5.49, 3.1, 1.8, 4.8, -6.4, 0.5, 5.0, -0.27, -1.62]
                }
                cn_sec = pd.DataFrame(cn_data)
                st.plotly_chart(px.bar(cn_sec.sort_values("YTD (%)"), x="YTD (%)", y="Sector", orientation='h', title="CN Sectors", template="plotly_dark", height=450), use_container_width=True)
