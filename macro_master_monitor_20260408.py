...
def draw_sparkline(df, column_name, title, color, hline_zero=False, hline_mean=False):
    ...
    if hline_zero:
        fig.add_hline(y=0, line_dash="solid", line_color="red", line_width=1, annotation_text="倒挂界限")
    if hline_mean:
...
with st.sidebar:
    ...
    if "✅" in data_bundle.get("status", ""):
        ...
        st.caption(f"包含近 2500 个交易日的时序数据。")
    else:
...
if "✅" in data_bundle.get("status", ""):
    ...
    with col1:
        ...
        # 核心改动：引入 Tabs 完美保留历史设计
        tab1, tab2 = st.tabs(["📈 10年历史趋势", "📊 流动性微观细节"])
        with tab1:
            st.plotly_chart(draw_sparkline(history, '10Y-2Y Spread', "US Treasury 10Y-2Y Spread", "#FF4B4B", hline_zero=True), width="stretch")
        with tab2:
            st.caption(f"Historical Context: Min -0.4% | Max 0.4% | Current: {spread:.3f}%")
    ...
    with col2:
        ...
        with tab1:
            st.plotly_chart(draw_sparkline(history, 'S&P 500', "S&P 500 Index Level", "#00CC96"), width="stretch")
        with tab2:
            # 完美保留上一版的条形图
    ...
    with col3:
        ...
        with tab1:
            st.plotly_chart(draw_sparkline(history, 'USD/CNH', "USD/CNH Exchange Rate", "#FFA15A"), width="stretch")
        with tab2:
            # 完美保留上一版的基准表格，并加入最新手动输入的 China 10Y
    ...
    with col4:
        ...
        with tab1:
            st.plotly_chart(draw_sparkline(history, 'Gold/WTI', "Gold-WTI Ratio (Recession Pricing)", "#AB63FA", hline_mean=True), width="stretch")
        with tab2:
            # 完美保留上一版的库存备忘录
