...
# ==========================================
st.set_page_config(
    page_title="Macro Terminal V2.2", 
    layout="wide", 
...
def fetch_global_data():
    ...
    yf_tickers = [
        ...
        # US GICS Level 1 Sector ETFs (SPDR)
...
if db:
    ...
    if page == "📈 Equity Markets":
        ...
        with tab2:
            gics_mapping = {
                'XLK': ('Information Technology', 29.6),
                'XLF': ('Financials', 13.1),
                'XLV': ('Health Care', 12.6),
                'XLY': ('Consumer Discretionary', 10.6),
                'XLC': ('Communication Services', 8.9),
                'XLI': ('Industrials', 8.8),
                'XLP': ('Consumer Staples', 6.1),
                'XLE': ('Energy', 3.9),
                'XLU': ('Utilities', 2.5),
                'XLB': ('Materials', 2.4),
                'XLRE': ('Real Estate', 2.3)
            }
            
            sector_data = []
            current_year = str(datetime.date.today().year)
            
            for ticker, (name, weight) in gics_mapping.items():
                df = yf_df.get(ticker)
                if df is not None and not df.empty:
                    try:
                        ytd_start_price = df.loc[current_year].iloc[0]['Close']
                        current_price = df.iloc[-1]['Close']
                        ytd_perf = ((current_price - ytd_start_price) / ytd_start_price) * 100
                        sector_data.append({'Sector': name, 'YTD (%)': ytd_perf, 'Weight': weight})
                    except Exception:
                        pass
            ...
            if sector_data:
                ...
                fig_tree = px.treemap(
                    df_sectors,
                    path=[px.Constant("S&P 500 GICS Sectors"), 'Sector'],
                    values='Weight',
                    color='YTD (%)',
                ...
                fig_tree.update_traces(
                    ...
                    customdata=df_sectors[['YTD (%)']],
                    hovertemplate="<b>%{label}</b><br>Weight: %{value}%<br>YTD: %{customdata[0]:.2f}%<extra></extra>",
                    root_color="#000000"
                )
                
                st.plotly_chart(fig_tree, use_container_width=True, config={'scrollZoom': False, 'displayModeBar': False})
            else:
                st.warning("GICS Sector Data is currently syncing. Please wait.")
    else:
        st.plotly_chart(draw_bloomberg_chart(target_df, selected_asset, color, selected_timeframe, show_ma=use_ma), use_container_width=True, config={'scrollZoom': True, 'displayModeBar': True})
