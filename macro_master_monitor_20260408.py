...
def fetch_global_data():
    ...
    # A. Yahoo Finance (Global Equities, FX, International Commodities)
    yf_tickers = [
...
def draw_chart(series_or_df, title, base_color):
    ...
    fig.update_layout(
        ...
        height=600,
        dragmode='pan', # Left-click drag to pan
        xaxis=dict(
            rangeslider=dict(visible=False), # Removed the small map at bottom
            type="date",
            showgrid=False
        ),
        yaxis=dict(
...
# ==========================================
# 5. Main Terminal Layout (Full Asset List)
# ==========================================
...
if db:
    ...
    # --- Module 3: FX & FI (FULL) ---
        ...
        fx_c = {
            ...
            "China 10Y Yield (AKShare)": (mk_df.get('China_10Y_Yield'), "#FF4B4B"), 
            "US Long Treas (TLT)": (yf_df.get('TLT'), "#4682B4")
        }
        render_grid(fx_c)

    # --- Module 4: Equity Markets (FULL) ---
